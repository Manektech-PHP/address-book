...
<html lang="en">
    ...
    <head>
        ...
        <link rel="shortcut icon" href="{{ asset('layouts/layout/img/favicon.ico')}}" /> </head>
		<style>
			#admin_logo{
				width:150px;
				max-width:100%;
    ...
    <body class=" login">
        ...
        <div class="content">
            <!-- BEGIN LOGIN FORM -->
            <form class="login-form" action="<?php echo route('login')?>" method="post">
			  @csrf
                <h3 class="form-title font-green">Sign In</h3>
                <div class="alert alert-danger display-hide">
                    <button class="close" data-close="alert"></button>
                    <span> Enter Valid Email-ID And Password. </span>
                </div>
                <div class="form-group">
                    <!--ie8, ie9 does not support html5 placeholder, so we just show field title for that-->
                    <label class="control-label visible-ie8 visible-ie9">Username</label>
                    <input class="form-control form-control-solid placeholder-no-fix {{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" required autofocus type="email" placeholder="Email" name="email" /> 
					@if ($errors->has('email'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control form-control-solid placeholder-no-fix{{ $errors->has('password') ? ' is-invalid' : '' }}" name="password" type="password" placeholder="Password" name="password" required>
